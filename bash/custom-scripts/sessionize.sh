...
select_dir_file_item() {
  ...
  if command -v fd &> /dev/null; then
    if command -v sed &> /dev/null; then
      selected_dir=$(fd . -H --type d --type f ~ | sed "s|$HOME|~|" | fzf --prompt="  Which directory are you looking for?   " --layout=reverse --height=~75% --border --exit-0 | sed "s|~|$HOME|") #Which directory to use fd . --type d --type f ~ | fzf
    elif command -v awk &> /dev/null; then
      selected_dir=$(fd . -H --type d --type f ~ | awk -v home="$HOME" '{gsub(home, "~"); print}' | fzf --prompt="  Which directory are you looking for?   " --layout=reverse --height=~75% --border --exit-0 | awk -v home="$HOME" '{gsub("~", home); print}') #Which directory to use fd . --type d --type f ~ | fzf
    else
      echo "Error: sed and awk is not available or not working properly. Please check it."
      selected_dir=$(fd . -H --type d --type f ~ | fzf --prompt="  Which directory are you looking for?   " --layout=reverse --height=~75% --border --exit-0) #Which directory to use fd . --type d --type f ~ | fzf
    fi
